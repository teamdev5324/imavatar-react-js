...
const OTPSigninSent = () => {
    ...
    const { values, errors, handleChange, handleSubmit, setFieldError, setFieldValue } = useFormik({
        ...
        onSubmit: async () => {
            ...
            var config = {
                ...
                url: 'http://35.170.79.161:8080/api/user/noAuth/loginByOTP',
                headers: {
    ...
    const sendOtp = async () => {
        ...
        api.post('http://35.170.79.161:8080/api/user/MP/noAuth/sendOTP/login', params)
            .then(async res => {
                res = res.data;
                console.log(res);
                setUid(res.data);
                ...
                if (res.status === 'SUCCESS') {
                    ...
                    // let res2 = await axios.get(`http://35.170.79.161:8080/api/user/noAuth/getUserInfo/${res.data}`);
                    // res2 = res2.data.data;
                    // console.log(res2);   
                                    ...
                                    <form>
                                        ...
                                        {resentActive && resentActive !== 0 ? (
                                            <div className="col-md-12 frmss mb-2">
                                                <label for="checkbox">Code expires in {resentActive === 60 ? '01:00' : resentActive < 10 ? '00:' + '0' + resentActive : '00:' + resentActive}</label>
                                            </div>
                                        ) : null}
                                        {resentActive === 0 && (
                                            <div className="col-md-12 frmss mb-3">
                                                <p className="resend"><a onClick={() => sendOtp()}>Resend OTP</a> </p>
                                            </div>
                                        )}

                                        <div className="col-md-12 frmss mb-4">
                                            <input type="button" className="btn btn1" value="Confirm" onClick={() => handleSubmit()} />
                                        </div>
                                    </form>
                                </div>
                            </div>
