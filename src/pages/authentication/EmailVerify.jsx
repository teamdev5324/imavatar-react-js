  ...
  const {
    ...
    onSubmit: async () => {
      ...
      const { data } = await axios.get(
        `http://52.90.60.5:8080/api/user/userVerification/email/${otp}/${userId}`
      );
      console.log(data);
        ...
        axios
          .get(
            'http://52.90.60.5:8080/api/user/noAuth/getUserInfo/' +
              JSON.parse(decrypt(localStorage.getItem('userData'))).id
          )
          .then(async (res) => {
            ...
            onSubmit={async (e) => {
              ...
              if (res__.data.status === 'SUCCESS') {
                ...
                const data = JSON.stringify({
                  ...
                  user_id: JSON.parse(decrypt(localStorage.getItem('userData')))
                    .id,
                });
